...
def training_loop(train_dataloader, opts):
    ...
    for _ in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # TOP K SCORE ADDITION
            # Find how realistic the discrimiantor thinks the images are:
            k = 8

            D_realistic_scores = D(fake_images).view(-1)

            sorted_scores, sorted_indeces = torch.sort(D_realistic_scores, descending=True)
            top_k_most_realistic = sorted_scores[:k]
            ...
            # now G loss is only based on the top k realistic images
            G_loss = torch.nn.BCEWithLogitsLoss()(top_k_most_realistic, torch.ones(top_k_most_realistic.shape))
...
def create_parser():
    ...
    parser.add_argument('--checkpoint_dir', default='./checkpoints_TOPK')
    parser.add_argument('--sample_dir', type=str, default='./TOPK')
